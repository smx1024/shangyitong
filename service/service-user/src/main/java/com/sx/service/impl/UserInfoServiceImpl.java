...
import com.sx.yygh.enums.AuthStatusEnum;
...
public class UserInfoServiceImpl extends ServiceImpl<UserInfoMapper, UserInfo> implements UserInfoService {
    ...
    public Map<String, Object> login(LoginVo loginVo) {
        ...
        //验证码验证成功，判断是否是第一次登录，如果是第一次登录，就自动注册登录
        //如果不是第一次登录，直接登录
        if (userInfo == null) {
            //根据手机号码查询数据库，判断是否是第一次登录
            userInfo = getOne(new QueryWrapper<UserInfo>().eq("phone", loginVo.getPhone()));
            if (userInfo==null){
                //注册
...
//            UpdateWrapper<UserInfo> updateWrapper = new UpdateWrapper<>().eq("openId", userId).set("status", status);
//            QueryWrapper<UserInfo> wrapper = new QueryWrapper<>();
            UserInfo userInfo = baseMapper.selectById(userId);
